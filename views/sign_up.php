...
<body class="flex h-screen bg-main">
    <form class="bg-white w-560px flex flex-col items-center m-auto py-30px radius-5px" action="../vendor/controllers/SignUpController.php" method="POST">
        ...
        <div class=" flex flex-col items-end">
            <input class="sign_field w-367px p-6px radius-5px border-solid-gray" type="text" name="login" placeholder="Введите логин" value="<?php if(isset($_SESSION['old_data']['login'])) echo $_SESSION['old_data']['login'] ?>">
            <p class="sign_errors text-right color-red w-367px"><?php if(isset($_SESSION['errors_msg']['login'])) echo $_SESSION['errors_msg']['login'][0] ?></p>
        </div>
        ...
        <div class="mt-10px flex flex-col items-end">
            <input class=" sign_field w-367px p-6px radius-5px border-solid-gray" type="password" name="password_confirm" placeholder="Введите пароль повторно">
            <p class="sign_errors text-right color-red w-367px"><?php if(isset($_SESSION['errors_msg']['password_confirm'])) echo $_SESSION['errors_msg']['password_confirm'][0] ?></p>
        </div>
