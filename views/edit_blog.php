...
<body class="h-screen bg-main flex">
    ...
    <form action="../vendor/controllers/EditBlogController.php?blog_id=<?= urlencode($_GET['blog_id']) ?>" method="POST" class="w-560px py-30px bg-white radius-5px m-auto flex flex-col items-center">
        ...
        <div class="create_buts w-406px flex justify-between">
            <button type="submit" name="action" value="delete" value="true" class="text-base bg-main p-10px border-none radius-5px">Удалить блог</button>
            <button type="submit" name="action" value="edit" value="true" class="text-base bg-main p-10px border-none radius-5px">Редактировать</button>
        </div>
    </form>
</body>
</html>
